...
export default function NeighborsPage() {
    ...
    const incomingRequests = friendRequests.filter(req => req.toUserId === currentUser?.uid && req.status === 'pending');
    const friends = allNeighbors.filter(n => userDetails?.friends?.some(f => f.uid === n.uid));
